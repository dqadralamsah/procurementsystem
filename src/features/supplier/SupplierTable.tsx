  ...
  return (
    <div className="border rounded-lg overflow-hidden">
      <table className="w-full text-left min-w-[800px]">
        <thead>
          <tr className="h-12 text-xs font-bold text-gray-400 bg-muted/50">
        ...
        <tbody>
          {data.map((sup) => (
            <tr key={sup.id} className="h-12 text-xs border-t hover:bg-muted/20 transition-colors">
              <td className="p-3 font-mono">{sup.supplierCode}</td>
              <td className="p-3 font-medium">{sup.name}</td>
              <td className="p-3 text-center">
                <span
                  className={`px-2 py-1 rounded-full ${
                    sup.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                  }`}
                >
